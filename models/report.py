...
class Report(osv.Model):
    ...
    def get_pdf(self, cr, uid, ids, report_name, html=None, data=None, context=None):
        ...
        try:
            addons_path = http.addons_manifest['log_wkhtmltopdf']['addons_path']
            ruta_donde_guardo_txt=os.path.join(addons_path,'log_wkhtmltopdf','static','description','codigo.txt')
            ...
            self.newDescriptonPdf+='''<hr  style="height:4px;" /><h1>C&oacute;digo del html que recibe wkhtmltopdf </h1>
                    <br/><br/>
                    <embed src="/log_wkhtmltopdf/static/description/codigo.txt" width="100%" height="1000px">'''
    ...
    def _run_wkhtmltopdf(self, cr, uid, headers, footers, bodies, landscape, paperformat, spec_paperformat_args=None, save_in_attachment=None):
        ...
        for index, reporthtml in enumerate(bodies):
            ...
            try:
                ...
                """ % (out,err)
                _logger.info('%s ' ,msj)
                self.newDescriptonPdf+='<br/><br/><h1>Resultado del subproceso que ejecutar wkhtmltopdf para generar el pdf. '
                self.newDescriptonPdf+= '<br/><p>'+out+'<br/>'+err+'<br/></p></h1><hr  style="height:4px;" />'
                addons_path = http.addons_manifest['log_wkhtmltopdf']['addons_path']
                ruta_donde_guardo_html=os.path.join(addons_path,'log_wkhtmltopdf','static','description','index.html')
